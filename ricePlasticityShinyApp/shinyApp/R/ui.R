library(shiny); library(shinydashboard); library(shinydashboardPlus); library(yaml); library(DT); library(shinyWidgets); library(shinyBS); library(shinyjs)
# library(plotly); library(visNetwork);
...
data_ui <- function(id, deg = FALSE) {
  ...
  if (deg == FALSE) box(width = 12, title = "Data (replicates aggregated)", closable = FALSE, solidHeader = TRUE, collapsible = TRUE, enable_sidebar = FALSE, status = "primary", enable_dropdown = FALSE,
    tabsetPanel(type = "pills", id=NULL, selected="dTabSel",
      ...
      radioButtons(inputId=ns('scale'), label='Scale by', choices=c('No', 'Row', 'Column'), selected='Row', inline=TRUE), '', 
...
upload_ui <- function(id, search.ui) {
   ...
   tabPanel("Landing Page", value='upload',
      ...
      #t3ags$div(class='tp', span(class='tpt', 'Ensure "columns in the data matrix corresponds with "rows" in the targets file respectively.'),
...
network_ui <- function(id) {
  ...
  tabPanel("Matrix Heatmap", id = 'mhm', icon = icon('border-all'),
      ...
      radioButtons(inputId=ns("mat.scale"), label="Scale by row", choices=c("No"='No', "Yes"='Row'), selected='', inline=TRUE), '',
