...
shinyServer(function(input, output, session) {
  ...
  observe({ 
    ...
    output$dld.st <- downloadHandler(   
      filename=function(){ "spatial_temporal_aSVG_data.zip" },
content=function(file=paste0(normalizePath(tempdir(check=TRUE), winslash="/", mustWork=FALSE), '/spatial_temporal_aSVG_data.zip')){ zip(file, c(dld.exp$st$data, dld.exp$st$svg)) }
  )
  })
 ...
 output$shm.ui <- renderUI({
      ...
      box(title="Spatial-Temporal Heatmap", status="primary", solidHeader=TRUE, collapsible=TRUE, width=ifelse(input$hide.lgd=='No', 9, 12), height=NULL, 
