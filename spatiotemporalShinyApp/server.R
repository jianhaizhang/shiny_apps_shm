...
funs <- list.files('R', '.R$', full.names=TRUE)
no <- lapply(funs, source)
...
shinyServer(function(input, output, session) {
  ...
  observe({
    ...
    library(gridExtra); library(ggdendro); 
    # library(WGCNA); library(flashClust); library(visNetwork);
    library(grid); library(xml2); library(plotly); library(data.table); library(genefilter);  
    incProgress(0.9, detail="in progress...")
    # library(HDF5Array); library(igraph)
    library(reshape2); library(animation); library(av); library(shinyWidgets); library(yaml); library(sortable)
  })
    ...
    dat.nas <- c(names(dat.def)) # For STHM.
    # updateSelectInput(session, 'fileIn', 'Step 1: data sets', dat.nas, lis.par$default.dataset)
    updateSelectInput(session, 'fileIn', 'Data set', dat.nas, lis.par$default.dataset) # For STHM.
...
content=function(file=paste0(normalizePath(tempdir(check=TRUE), winslash="/", mustWork=FALSE), '/spatiotemporal_aSVG_data.zip')){ zip(file, c(dld.exp$st$data, dld.exp$st$svg)) }
 ...
 output$shm.ui <- renderUI({
      ...
      box(title="Spatiotemporal Heatmap", status="primary", solidHeader=TRUE, collapsible=TRUE, width=ifelse(input$hide.lgd=='No', 9, 12), height=NULL, 
